...
func (p *progressMiddleware) BeforeAgent(ctx context.Context, st *middleware.State) error {
	// 每条 message 开始时重置透传状态。message 生命周期 = BeforeAgent → 0 或多次
	// completeOnce（升级/压缩）→ AfterAgent。在 message 边界重置避免跨 message 残留。
	if state, ok := ctx.Value(streamForwardStateCtxKey).(*streamForwardState); ok && state != nil {
		state.forwarded.Store(false)
...
func (p *progressMiddleware) AfterAgent(ctx context.Context, st *middleware.State) error {
	...
	idx := 0
	// 底层已透传真 delta 时：text 块（start+delta）已在回调中发出，此处只补发 stop，
	// 跳过 textBlock 假重放，并把后续 tool 块 index 顺延到 1（与非 forwarded 路径一致）。
	forwarded := false
	if state, ok := ctx.Value(streamForwardStateCtxKey).(*streamForwardState); ok && state != nil {
		forwarded = state.forwarded.Load()
		if forwarded && state.started.Load() {
			stopIdx := 0
			p.emit(ctx, StreamEvent{Type: EventContentBlockStop, Index: &stopIdx})
			idx = 1
		}
	}
	if !forwarded {
