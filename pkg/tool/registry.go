...
// MCPServerInfo describes a registered MCP server for management/inspection.
type MCPServerInfo struct {
	Name      string
	ServerID  string
	Type      string // "stdio" | "sse" | "http" | "unknown" (derived from spec)
	ToolCount int
}
...
func (r *Registry) ListMCPServers() []MCPServerInfo {
	r.mu.RLock()
	defer r.mu.RUnlock()
	result := make([]MCPServerInfo, 0, len(r.mcpSessions))
	for _, info := range r.mcpSessions {
		...
		result = append(result, MCPServerInfo{
			...
			Type:      mcpServerTypeFromSpec(info.serverID),
...
// (the spec/path used during registration). All tools belonging to this
// server are also removed from the registry. The session is closed outside
// the registry lock so a slow close cannot block other operations.
func (r *Registry) RemoveMCPServer(serverID string) error {
	...
	r.mu.Lock()
	idx := -1
	for i, info := range r.mcpSessions {
		if info != nil && info.serverID == serverID {
			idx = i
			break
		}
	}
	...
	info := r.mcpSessions[idx]
	for toolName := range info.toolNames {
		delete(r.tools, toolName)
	}
	r.mcpSessions = append(r.mcpSessions[:idx], r.mcpSessions[idx+1:]...)
	r.mu.Unlock()
...
// mcpServerTypeFromSpec classifies a server spec the same way the SDK's
// transport layer (mcp.buildSessionTransport) does, so the admin display never
// misrepresents how a server is actually wired.
func mcpServerTypeFromSpec(spec string) string {
	...
	switch {
	case strings.HasPrefix(lowered, "stdio://"):
		return "stdio"
	case strings.HasPrefix(lowered, "sse://"):
		return "sse"
	}
	// HTTP-family specs carry the transport kind in the scheme hint, e.g.
	// "http+stream://..." → http, "http+sse://..." → sse.
	if u, err := url.Parse(lowered); err == nil && u.Scheme != "" {
		base, hint, hasHint := strings.Cut(u.Scheme, "+")
		if hasHint && (base == "http" || base == "https") {
			...
			case "stream", "streamable", "http", "json":
				return "http"
			default:
				// mcp.go's parseHTTPFamilySpec rejects unknown hints; such a
				// spec cannot become a registered server. Report "unknown"
				// rather than fall through to a misleading classification.
				return "unknown"
			}
		}
	}
	// Plain http(s) URLs (no hint) wire up as SSE in the SDK.
	if strings.HasPrefix(lowered, "http://") || strings.HasPrefix(lowered, "https://") {
		return "sse"
	}
	// Fallback: anything else is treated as a bare stdio command.
	return "stdio"
}
